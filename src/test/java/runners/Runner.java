...
@CucumberOptions(
        ...
        glue = "stepdefinition",
        tags="@msk", // butunu calistirmak icin @all kullanilabilir
        dryRun = false
)       // tags tagina yapilan gruplarin isim veya isimleri yazilirsa sadece onlari calistirir.
...
Glue kısmı  ile birbirine yapıştırıyoruz, yani feauture fileım ile
stepDefinition umu birbirine yapıştırıyorum
Daha sonra tag kısmında feature içinde olusturdugum isimlendirme
kısmını burada çağırıyorum.
dryRun(prova anlamında)= true ile calıstırdığımızda  eksik stepDefinitionları
bulmak için kullanıyoruz, browser ı açmadan toolumuza getirir.
